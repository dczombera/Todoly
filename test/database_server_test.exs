defmodule DatabaseServerTest do
  ...
  test "pooling" do
    assert(:ok == Todo.Database.store(1, [princess: "leia"]))
    assert(:ok == Todo.Database.store(2, [darth: "vader"]))
    
    assert([darth: "vader"]   == Todo.Database.get(2))
    assert([princess: "leia"] == Todo.Database.get(1))
  end
end
